...
function WishlistIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path d="M20.8 5a5.4 5.4 0 0 0-7.6 0L12 6.2 10.8 5a5.4 5.4 0 0 0-7.6 7.6l1.2 1.2L12 21.4l7.6-7.6 1.2-1.2a5.4 5.4 0 0 0 0-7.6Z" />
    </svg>
  )
}
...
export function Header({
  ...
  const [wishlistBadgeCount, setWishlistBadgeCount] = useState(null)
  ...
  useEffect(() => {
    ...
    hydrateCartCount()
    const unsubscribe = subscribeCartUpdated(hydrateCartCount)
    const onFocus = () => hydrateCartCount()
  ...
  }, [cartCount, enableCartBadge])

  useEffect(() => {
    if (!actionItems.some((item) => item.icon === 'wishlist')) {
      setWishlistBadgeCount(null)
      return
    }

    const sync = () => {
      const count = getWishlistCount()
      setWishlistBadgeCount(count > 0 ? count : null)
    }
  ...
  const decoratedActionItems = useMemo(
    ...
    [actionItems, cartBadgeCount, enableCartBadge, wishlistBadgeCount]
  )
    ...
    <div className="enterprise-header-shell">
      ...
      <header className="enterprise-header" role="banner">
        <div className="enterprise-header-inner">
          ...
          <div className="enterprise-utility">
            <HeaderSearch />
