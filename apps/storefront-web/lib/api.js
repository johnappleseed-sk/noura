import { commerceApiClient, commerceApiHost } from './apiClient'
...
function resolveMediaUrl(url) {
  ...
  const host = String(commerceApiHost || '').replace(/\/+$/, '')
  if (!host) {
    return normalized
  }
...
function normalizeProductCard(product) {
  ...
  return {
    ...
    imageUrl: resolveMediaUrl(product.imageUrl || primaryMedia?.url || null),
...
function normalizeRecommendationCard(product) {
  return {
    ...
    name: product?.name || '',
    price: product?.price || 0,
    imageUrl: resolveMediaUrl(product?.imageUrl || null),
    categoryId: product?.categoryId || null,
...
function normalizeProductDetail(product) {
  ...
  return {
    ...
    name: product.name,
    price: product.price || 0,
    imageUrl: resolveMediaUrl(primaryMedia?.url || null),
...
export async function getProducts({ q, categoryId, storeId, page = 0, size = 12, sort = 'featured' } = {}) {
  const params = new URLSearchParams()
