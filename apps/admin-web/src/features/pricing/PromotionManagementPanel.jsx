...
export function PromotionManagementPanel() {
  ...
  const [promotionPager, setPromotionPager] = useState({ page: 0, size: 10 })
  ...
  async function load(nextFilters = filters) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to load promotions.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const totalPromotionPages = Math.max(1, Math.ceil(promotions.length / promotionPager.size))
  const safePromotionPage = Math.min(promotionPager.page, totalPromotionPages - 1)
  const visiblePromotions = useMemo(() => {
    const start = safePromotionPage * promotionPager.size
    return promotions.slice(start, start + promotionPager.size)
  }, [promotions, safePromotionPage, promotionPager.size])

  useEffect(() => {
    if (promotionPager.page <= totalPromotionPages - 1) return
    setPromotionPager((current) => ({ ...current, page: Math.max(0, totalPromotionPages - 1) }))
  }, [promotionPager.page, totalPromotionPages])
  ...
  async function togglePromotion(promotion, patch) {
    const isArchiving = patch.archived === true && !promotion.archived
    const isDeactivating = patch.active === false && promotion.active && !isArchiving
    if (isArchiving || isDeactivating) {
      const confirmed = await confirm({
        title: isArchiving ? 'Archive promotion?' : 'Deactivate promotion?',
        message: isArchiving
          ? 'Archived promotions are hidden from active promotion views and can be restored later.'
          : 'Deactivated promotions stop being applied in new pricing evaluations.',
        description: promotion.name,
        confirmLabel: isArchiving ? 'Archive promotion' : 'Deactivate promotion'
      })
      if (!confirmed) return
    }
    ...
    try {
      ...
      if (isArchiving) {
        setFlash('Promotion archived.')
      } else if (patch.archived === false && promotion.archived) {
        setFlash('Promotion restored.')
      } else if (isDeactivating) {
        setFlash('Promotion deactivated.')
      } else if (patch.active === true && !promotion.active) {
        setFlash('Promotion activated.')
      } else {
  ...
  return (
    <section className="panel">
      ...
      <form
        ...
        onSubmit={(event) => {
          event.preventDefault()
          setPromotionPager((current) => ({ ...current, page: 0 }))
          load(filters)
        }}
      ...
      >
        <label className="toolbar-field toolbar-grow">
          <span>Search</span>
          <input className="toolbar-control" value={filters.query} onChange={(event) => setFilters((current) => ({ ...current, query: event.target.value }))} placeholder="Name or code" />
        </label>
        <label className="toolbar-field">
          <span>Active</span>
          <select className="toolbar-control" value={filters.active} onChange={(event) => setFilters((current) => ({ ...current, active: event.target.value }))}>
        ...
        <label className="toolbar-field">
          <span>Archived</span>
          <select className="toolbar-control" value={filters.archived} onChange={(event) => setFilters((current) => ({ ...current, archived: event.target.value }))}>
            <option value="">All</option>
        ...
        <div className="toolbar-actions">
          <button className="btn btn-primary toolbar-btn" type="submit">Apply</button>
        </div>
      </form>
      ...
      <div className="table-card" style={{ marginBottom: 28 }}>
        <PaginationControls
          ...
          first={safePromotionPage <= 0}
          last={safePromotionPage >= totalPromotionPages - 1}
          pageSize={promotionPager.size}
          onPageSizeChange={(nextSize) => setPromotionPager({ page: 0, size: nextSize })}
          onPrev={() => setPromotionPager((current) => ({ ...current, page: Math.max(0, current.page - 1) }))}
          onNext={() => setPromotionPager((current) => ({ ...current, page: Math.min(totalPromotionPages - 1, current.page + 1) }))}
          noun="promotions"
        />
