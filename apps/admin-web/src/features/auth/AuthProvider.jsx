...
function resolveSessionRoles(profileRoles, snapshotRoles, capabilityRoles) {
  const preferred = Array.isArray(capabilityRoles) && capabilityRoles.length
    ? capabilityRoles
    : (Array.isArray(profileRoles) && profileRoles.length ? profileRoles : snapshotRoles)
  return normalizeRoles(preferred)
}
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      try {
        ...
        if (!active) return
        const roles = resolveSessionRoles(profile.roles, snapshot.roles, capabilitiesResponse?.roles)
        const merged = {
          ...snapshot,
