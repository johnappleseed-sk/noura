...
export function MovementsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load movement workspace.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to submit movement.'))
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="page">
      ...
      {!canManage ? (
        <div className="panel" style={{ marginBottom: 14 }}>
          <p className="subtle-meta">Your role is read-only in this workspace. Managers can view history here, but only admins and warehouse managers can submit movements.</p>
        </div>
      ) : null}
