...
export function BatchesPage() {
  ...
  async function loadReferences() {
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load products for batch filters.'))
    }
  }
  ...
  async function load({ nextPage = batchPage.page, nextFilters = filters } = {}) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load batch lots.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    ...
    getBatchLot(selectedId)
      ...
      .catch((err) => {
        if (cancelled) return
        setError(getErrorMessage(err, 'Failed to load batch lot detail.'))
      })
      .finally(() => {
        if (cancelled) return
