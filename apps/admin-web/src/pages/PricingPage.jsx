...
export function PricingPage() {
  ...
  const [priceListPager, setPriceListPager] = useState({ page: 0, size: 10 })
  const [activePromotionPager, setActivePromotionPager] = useState({ page: 0, size: 10 })
  ...
  const totalPriceListPages = Math.max(1, Math.ceil(priceListOptions.length / priceListPager.size))
  const safePriceListPage = Math.min(priceListPager.page, totalPriceListPages - 1)
  const visiblePriceLists = useMemo(() => {
    const start = safePriceListPage * priceListPager.size
    return priceListOptions.slice(start, start + priceListPager.size)
  }, [priceListOptions, safePriceListPage, priceListPager.size])

  const totalActivePromotionPages = Math.max(1, Math.ceil(promotions.length / activePromotionPager.size))
  const safeActivePromotionPage = Math.min(activePromotionPager.page, totalActivePromotionPages - 1)
  const visibleActivePromotions = useMemo(() => {
    const start = safeActivePromotionPage * activePromotionPager.size
    return promotions.slice(start, start + activePromotionPager.size)
  }, [promotions, safeActivePromotionPage, activePromotionPager.size])

  useEffect(() => {
    if (priceListPager.page <= totalPriceListPages - 1) return
    setPriceListPager((current) => ({ ...current, page: Math.max(0, totalPriceListPages - 1) }))
  }, [priceListPager.page, totalPriceListPages])

  useEffect(() => {
    if (activePromotionPager.page <= totalActivePromotionPages - 1) return
    setActivePromotionPager((current) => ({ ...current, page: Math.max(0, totalActivePromotionPages - 1) }))
  }, [activePromotionPager.page, totalActivePromotionPages])
  ...
  async function load() {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load pricing workspace.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleCreatePriceList(event) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to create price list.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleUpsertPrice(event) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to upsert price.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleQuote(event) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to quote variant price.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleCreatePromotion(event) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to create promotion.'))
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="page">
      ...
      <div className="panel-grid">
        <section className="panel">
          ...
          {priceLists.length ? (
            <>
              <PaginationControls
                page={safePriceListPage}
                totalPages={totalPriceListPages}
                totalElements={priceListOptions.length}
                first={safePriceListPage <= 0}
                last={safePriceListPage >= totalPriceListPages - 1}
                pageSize={priceListPager.size}
                onPageSizeChange={(nextSize) => setPriceListPager({ page: 0, size: nextSize })}
                onPrev={() => setPriceListPager((current) => ({ ...current, page: Math.max(0, current.page - 1) }))}
                onNext={() => setPriceListPager((current) => ({ ...current, page: Math.min(totalPriceListPages - 1, current.page + 1) }))}
                noun="price lists"
              />
      ...
      <section className="panel">
        ...
        {promotions.length ? (
          <>
            <PaginationControls
              page={safeActivePromotionPage}
              totalPages={totalActivePromotionPages}
              totalElements={promotions.length}
              first={safeActivePromotionPage <= 0}
              last={safeActivePromotionPage >= totalActivePromotionPages - 1}
              pageSize={activePromotionPager.size}
              onPageSizeChange={(nextSize) => setActivePromotionPager({ page: 0, size: nextSize })}
              onPrev={() => setActivePromotionPager((current) => ({ ...current, page: Math.max(0, current.page - 1) }))}
              onNext={() => setActivePromotionPager((current) => ({ ...current, page: Math.min(totalActivePromotionPages - 1, current.page + 1) }))}
              noun="active promotions"
            />
            <div className="table-wrap">
              <table>
                <thead>
                ...
                <tbody>
                  {visibleActivePromotions.map((promo) => (
                    <tr key={promo.id}>
                      <td>
