...
export function CarouselsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load carousel slides.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function editCarousel(carouselId) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to load carousel detail.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function saveDraft() {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to save carousel.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function togglePublish(slide) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to change publish state.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function quickStatus(slide, status) {
    const isArchiving = status === 'ARCHIVED' && slide.status !== 'ARCHIVED'
    if (isArchiving) {
      const confirmed = await confirm({
        title: 'Archive carousel slide?',
        message: 'Archiving removes this slide from active storefront rotation without deleting it.',
        description: slide.title || slide.id,
  ...
  async function reorderPageRows(sourceId, targetId) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to reorder carousel slides.'))
    } finally {
      setSaving(false)
      setDraggedId(null)
    }
                        ...
                        <div className="inline-actions wrap">
                          ...
                          {slide.deletedAt ? (
                            ...
                            <button
                              ...
                              onClick={async () => {
                                const confirmed = await confirm({
                                  title: 'Move carousel slide to trash?',
                                  message: 'This action moves the slide to trash and removes it from storefront APIs immediately.',
                                  description: slide.title || slide.id,
                                  confirmLabel: 'Move to trash'
                                })
                                if (!confirmed) return
                                await runInlineAction(() => deleteCarousel(slide.id), 'Carousel moved to trash.')
                              }}
                            >
                              Move to trash
                            </button>
                ...
                onClick={async () => {
                  const confirmed = await confirm({
                    ...
                    message: draft.published
                      ? 'Unpublish this slide and keep it available in admin draft mode.'
                      : 'Publish this slide using the current schedule window.',
                    description: draft.title || editingId,
                    confirmLabel: draft.published ? 'Unpublish' : 'Publish'
                  })
                  if (!confirmed) return
                  await runInlineAction(
                    () => publishCarousel(editingId, {
                      published: !draft.published,
                      startAt: toIsoOrNull(draft.startAt),
                      endAt: toIsoOrNull(draft.endAt)
                    }),
                    draft.published ? 'Carousel unpublished.' : 'Carousel published.',
                    editingId
                  )
                }}
              >
