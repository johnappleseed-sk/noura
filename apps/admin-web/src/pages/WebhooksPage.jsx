...
export function WebhooksPage() {
  ...
  async function load(nextSelectedId = selectedId) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load webhook subscriptions.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSave(event) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to save webhook subscription.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleDelete() {
    ...
    const confirmed = await confirm({
      title: 'Move webhook subscription to trash?',
      message: 'This action moves the selected webhook subscription to trash. You can restore it from the Recovery Center.',
      description: subscriptionLabel(selected),
    ...
    try {
      ...
      setFlash('Webhook subscription moved to trash.')
      setSelectedId('')
      setForm(DEFAULT_FORM)
      await load('')
    } catch (err) {
      setError(getErrorMessage(err, 'Failed to move webhook subscription to trash.'))
    } finally {
      setDeletingId('')
    }
  }
