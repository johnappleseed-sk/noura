...
export function LocationsPage() {
  ...
  async function load(nextWarehouseId = selectedWarehouseId) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load location workspace.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function removeWarehouse() {
    ...
    const confirmed = await confirm({
      title: 'Move warehouse to trash?',
      message: 'This action moves the warehouse to trash. You can restore it later from the Recovery Center.',
      description: selectedWarehouse?.name || selectedWarehouseId,
    ...
    try {
      ...
      setFlash('Warehouse moved to trash.')
      setSelectedWarehouseId('')
      setWarehouseForm(DEFAULT_WAREHOUSE_FORM)
      await load('')
    } catch (err) {
      setError(getErrorMessage(err, 'Failed to move warehouse to trash.'))
    } finally {
      setWarehouseSaving(false)
    }
  }
  ...
  async function saveBin(event) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to save bin.'))
    } finally {
      setBinSaving(false)
    }
  }
  ...
  async function removeBin() {
    ...
    const confirmed = await confirm({
      title: 'Move bin to trash?',
      message: 'This action moves the selected bin to trash. You can restore it later from the Recovery Center.',
      description: selectedBinId,
      confirmLabel: 'Move to trash'
    })
    if (!confirmed) return
    setBinSaving(true)
    ...
    try {
      ...
      setFlash('Bin moved to trash.')
      setSelectedBinId('')
      setBinForm(DEFAULT_BIN_FORM)
      await load(selectedWarehouseId)
    } catch (err) {
      setError(getErrorMessage(err, 'Failed to move bin to trash.'))
    } finally {
      setBinSaving(false)
    }
  }
    ...
    <div className="page">
      ...
      {!canManage ? (
        <div className="panel" style={{ marginBottom: 18 }}>
          <p className="subtle-meta">Your role is read-only in this workspace. Use an admin or warehouse-manager account to manage warehouses and bins.</p>
        </div>
      ) : null}
