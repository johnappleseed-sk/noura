...
function validationBorderColor(result) {
...
export function ServiceAreasPage() {
  ...
  async function load(nextSelectedId = selectedServiceAreaId) {
    ...
    try {
      ...
      setError(getErrorMessage(requestError, 'Failed to load service area workspace.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function saveServiceArea(event) {
    ...
    try {
      ...
      setError(getErrorMessage(requestError, 'Unable to save service area.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function toggleServiceAreaStatus(area) {
    ...
    try {
      ...
      setError(getErrorMessage(requestError, 'Unable to update service area status.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function removeServiceArea() {
    ...
    const confirmed = await confirm({
      title: 'Move service area to trash?',
      message: 'Deleting a service area now moves it to the recovery center trash instead of hard-deleting it.',
      description: selectedServiceArea ? selectedServiceArea.name : 'Selected service area',
    ...
    try {
      ...
      setFlash('Service area moved to trash.')
      resetForm()
      await load('')
    } catch (requestError) {
      setError(getErrorMessage(requestError, 'Unable to move service area to trash.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  async function runValidation(event) {
    ...
    try {
      ...
      setError(getErrorMessage(requestError, 'Unable to validate coordinates.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="page">
      ...
      <section className="panel" style={{ marginTop: 20 }}>
        ...
        {validationResult ? (
          <div className="panel" style={{ marginTop: 18, borderLeft: `4px solid ${validationBorderColor(validationResult)}` }}>
