...
export function CatalogPage() {
  ...
  async function load(nextFilters = productFilters) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load catalog workspace.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  async function removeCategory() {
    ...
    const confirmed = await confirm({
      ...
      message: 'This action moves the selected category to trash. You can restore it later from the Recovery Center.',
      description: selectedCategory?.name || selectedCategoryId,
    ...
    setError('')
    try {
      await trashCategory(selectedCategoryId)
      setFlash('Category moved to trash.')
      resetCategoryForm()
      await load()
    } catch (err) {
      setError(getErrorMessage(err, 'Failed to move category to trash.'))
    } finally {
      setCategorySaving(false)
    }
  }
  ...
  async function moveProductToTrash(productId, productName = '') {
    if (!canManage || !productId) return
    const confirmed = await confirm({
      title: 'Move product to trash?',
      message: 'This action moves the selected product to trash. You can restore it later from the Recovery Center.',
      description: productName || selectedProduct?.name || productId,
      confirmLabel: 'Move to trash'
    })
    if (!confirmed) return
    setProductSaving(true)
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to move product to trash.'))
    } finally {
      setProductSaving(false)
    }
  }
  ...
  async function removeProduct() {
    await moveProductToTrash(selectedProductId, selectedProduct?.name || '')
  }
  ...
  return (
    <div className="page">
      ...
      {!canManage ? <p className="subtle-meta">Your role is read-only in this workspace. Switch to an admin account to create, update, or move catalog records to trash.</p> : null}
                      ...
                      <td>
                        <div className="action-group">
                          ...
                          <button
                            ...
                            type="button"
                            disabled={!canManage || productSaving}
                            onClick={() => moveProductToTrash(product.id, product.name)}
                            title="Move product to trash"
                          >
                            Trash
                          </button>
