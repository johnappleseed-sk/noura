...
export function SerialsPage() {
  ...
  async function loadReferences() {
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load reference lists for serial filters.'))
    }
  }
  ...
  async function loadBins(warehouseId) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load bins for warehouse filter.'))
    }
  }
  ...
  async function load({ nextPage = serialPage.page, nextFilters = filters } = {}) {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to load serial numbers.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    ...
    getSerialNumber(selectedId)
      ...
      .catch((err) => {
        if (cancelled) return
        setError(getErrorMessage(err, 'Failed to load serial detail.'))
      })
      .finally(() => {
        if (cancelled) return
    ...
    <div className="page">
      ...
      <section className="panel">
        <div className="section-head">
          <div>
            <h3>Filters</h3>
            <p>{serialPage.totalElements ? `${serialPage.totalElements} serial(s)` : 'No serials matched yet.'}</p>
          </div>
          <PaginationControls
            page={serialPage.page}
            totalPages={serialPage.totalPages}
            totalElements={serialPage.totalElements}
            first={serialPage.first}
            last={serialPage.last}
            pageSize={Number(filters.size || 20)}
            onPageSizeChange={(nextSize) => {
              const nextFilters = { ...filters, size: String(nextSize) }
              setFilters(nextFilters)
              load({ nextPage: 0, nextFilters })
            }}
            onPrev={goPrev}
            onNext={goNext}
            noun="serials"
