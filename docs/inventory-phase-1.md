...
- Database: PostgreSQL `15+` with Flyway
  - The inventory slice uses its own Flyway location and a dedicated PostgreSQL schema (`inventory`) inside the primary `noura` database so it can run without colliding with platform tables.
...
```json
{
  ...
  "datasourceUrl": "jdbc:postgresql://localhost:5432/noura?currentSchema=inventory",
...
3. Confirm Flyway creates the tables in schema `inventory` within database `noura`.
...
- HTTP `200`
- `status = "UP"`
- PostgreSQL schema `inventory` contains the baseline tables from `V1__initial_schema.sql`
