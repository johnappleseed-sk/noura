...
/**
 * Resolves request-level order actor context from gateway-forwarded headers.
 */
@Component
@RequiredArgsConstructor
public class OrderRequestContextResolver {
     ...
     */
    public OrderRequestContext resolve(HttpServletRequest request) {
        String subject = normalizeNullable(request.getHeader(SUBJECT_HEADER));
        if (subject == null) {
            subject = resolveAuthorizationFingerprint(normalizeNullable(request.getHeader(HttpHeaders.AUTHORIZATION)));
        }
        ...
        return new OrderRequestContext(subject, roles, internalCall);
    }
    ...
    /**
     * Resolves stable customer fallback from bearer token by hashing token value.
     *
     * @param authorization normalized authorization header
     * @return token fingerprint subject or null
     */
