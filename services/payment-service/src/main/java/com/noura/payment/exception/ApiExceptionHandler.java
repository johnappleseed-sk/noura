...
/**
 * Global exception-to-response mapper for payment APIs.
 */
...
public class ApiExceptionHandler {
    ...
    /**
     * Maps payment operation failures to configured status codes.
     *
     * @param ex payment operation exception
     * @param request current HTTP request
     * @return operation failure envelope
     */
    @ExceptionHandler(PaymentOperationException.class)
    public ResponseEntity<ApiResponse<Void>> handlePaymentOperation(PaymentOperationException ex, HttpServletRequest request) {
        return ResponseEntity.status(ex.getStatus()).body(ApiResponse.fail(
                "Payment operation rejected",
                ex.getCode(),
                ex.getMessage(),
                request.getRequestURI()
        ));
