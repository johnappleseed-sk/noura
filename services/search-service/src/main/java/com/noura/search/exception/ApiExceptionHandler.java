...
/**
 * Global exception-to-response mapper for search-service APIs.
 */
...
public class ApiExceptionHandler {
    ...
    /**
     * Maps enum/UUID/path conversion failures to HTTP 400.
     *
     * @param ex argument conversion exception
     * @param request current HTTP request
     * @return validation error envelope
     */
    ...
    /**
     * Maps search operation failures to configured status codes.
     *
     * @param ex search operation exception
     * @param request current HTTP request
     * @return operation failure envelope
     */
    @ExceptionHandler(SearchOperationException.class)
    public ResponseEntity<ApiResponse<Void>> handleSearchOperation(
            SearchOperationException ex,
            HttpServletRequest request
    ) {
        return ResponseEntity.status(ex.getStatus()).body(ApiResponse.fail(
                "Search operation rejected",
                ex.getCode(),
                ex.getMessage(),
                request.getRequestURI()
        ));
