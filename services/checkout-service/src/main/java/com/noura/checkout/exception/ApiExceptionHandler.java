...
/**
 * Global exception-to-response mapper for checkout APIs.
 */
...
public class ApiExceptionHandler {
     ...
     * @return validation failure envelope
     */
    ...
    /**
     * Maps not-found domain failures to HTTP 404.
     *
    ...
    /**
     * Maps checkout operation failures to declared HTTP status values.
     *
     * @param ex checkout operation exception
     * @param request current HTTP request
     * @return operation failure envelope
     */
    @ExceptionHandler(CheckoutOperationException.class)
    public ResponseEntity<ApiResponse<Void>> handleCheckoutOperation(CheckoutOperationException ex, HttpServletRequest request) {
        return ResponseEntity.status(ex.getStatus()).body(ApiResponse.fail(
                "Checkout operation rejected",
                ex.getCode(),
                ex.getMessage(),
                request.getRequestURI()
        ));
