...
/**
 * Global exception-to-response mapper for promotion-service APIs.
 */
...
public class ApiExceptionHandler {
    ...
    /**
     * Maps promotion operation failures to configured status codes.
     *
     * @param ex promotion operation exception
     * @param request current HTTP request
     * @return operation failure envelope
     */
    @ExceptionHandler(PromotionOperationException.class)
    public ResponseEntity<ApiResponse<Void>> handlePromotionOperation(
            PromotionOperationException ex,
            HttpServletRequest request
    ) {
        return ResponseEntity.status(ex.getStatus()).body(ApiResponse.fail(
                "Promotion operation rejected",
                ex.getCode(),
                ex.getMessage(),
                request.getRequestURI()
        ));
