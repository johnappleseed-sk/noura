...
## Platform Bootstrap Quick Start

The repository now includes a bootstrap foundation under `com.company.platform` with:
...
- security placeholder configuration
- OpenAPI bootstrap configuration
- Flyway bootstrap migration
...
- `local-postgres`: `application-local-postgres.yml`
...
```bash
psql -U saturn -h localhost -p 5432 -d postgres -c "CREATE DATABASE noura;"
psql -U saturn -h localhost -p 5432 -d noura -c "CREATE SCHEMA IF NOT EXISTS inventory;"
```

Use `backend/.env.example` as the local environment template.
...
Local profiles (`local`, `local-postgres`, `local-mysql`, `test`) emit warnings instead of failing.
...
- `GET /api/v1/admin/capabilities`
  ...
  - Guarded for authenticated sessions.
- `GET /api/v1/admin/authorization/matrix`
  - Returns the versioned enterprise role-permission matrix contract (actions, scopes, role grants, capability mappings).
  - Guarded for `ADMIN`/`SUPER_ADMIN`.
...
- `GET /api/v1/admin/authorization/audit-logs`
  - RBAC management endpoints (role CRUD, permission assignment, user-role assignment) and RBAC audit-log query.
  - Permission-aware endpoint guards with `ADMIN`/`SUPER_ADMIN` fallback:
    - `PERM_ROLES_READ`, `PERM_ROLES_CREATE`, `PERM_ROLES_UPDATE`, `PERM_ROLES_DELETE`, `PERM_ROLES_APPROVE`
    - `PERM_USERS_READ`, `PERM_USERS_UPDATE`
    - `PERM_AUDIT_LOGS_READ`

Persistence:
- RBAC tables and seed data are created by `db/migration/V20__admin_rbac_persistent_storage.sql`.
- RBAC audit log table is created by `db/migration/V21__admin_rbac_audit_logs.sql`.
