...
class ProductControllerSecurityIntegrationTest {
    ...
    @Test
    void searchProducts_shouldRejectNonAdminUser() throws Exception {
    ...
    @Test
    void searchProducts_shouldAllowAdminUser() throws Exception {
        mockMvc.perform(get("/api/v1/products/search")
    ...
    @Test
    void uploadsPath_shouldAllowAnonymousAccess() throws Exception {
        mockMvc.perform(get("/uploads/non-existent-image.webp"))
                .andExpect(status().isNotFound());
    }

    @Test
    void generateMissingFields_shouldRejectNonAdminUser() throws Exception {
        mockMvc.perform(post("/api/v1/products/{productId}/generate-missing", UUID.randomUUID())
                        .with(user("customer@noura.test").roles("CUSTOMER")))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.error.code").value("ACCESS_DENIED"));
    }
