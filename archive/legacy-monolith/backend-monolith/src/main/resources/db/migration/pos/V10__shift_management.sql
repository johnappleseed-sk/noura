...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_sale_shift'
    ) THEN
        ALTER TABLE sale
...
CREATE INDEX IF NOT EXISTS idx_shift_cash_event_shift ON shift_cash_event (shift_id);
CREATE INDEX IF NOT EXISTS idx_shift_cash_event_created_at ON shift_cash_event (created_at);
CREATE INDEX IF NOT EXISTS idx_shift_terminal_status ON shift (terminal_id, status);
CREATE INDEX IF NOT EXISTS idx_shift_opened_at ON shift (opened_at);
CREATE INDEX IF NOT EXISTS idx_shift_closed_at ON shift (closed_at);
CREATE INDEX IF NOT EXISTS idx_sale_shift ON sale (shift_id);
