...
public class InventoryServiceImpl implements InventoryService {
    ...
    public WarehouseDto createWarehouse(WarehouseRequest request) {
        ...
        return new WarehouseDto(saved.getId(), saved.getName(), saved.getLocation(), saved.getStore() == null ? null : saved.getStore().getId(), saved.isActive());
    }
    ...
    public List<WarehouseDto> warehouses() {
        return warehouseRepository.findAll().stream()
                .map(item -> new WarehouseDto(item.getId(), item.getName(), item.getLocation(), item.getStore() == null ? null : item.getStore().getId(), item.isActive()))
                .toList();
    }
