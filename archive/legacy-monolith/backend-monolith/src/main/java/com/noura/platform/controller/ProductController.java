...
public class ProductController {
    ...
    public ApiResponse<List<ProductSearchResultDto>> searchProducts(
        ...
        return ApiResponse.ok("Product search results", productEnrichmentService.searchExistingProducts(q), http.getRequestURI());
    }
    ...
    @PostMapping("/{productId}/generate-missing")
    public ApiResponse<ProductEnrichmentResponse> generateMissing(
            @PathVariable UUID productId,
            HttpServletRequest http
    ) {
        return ApiResponse.ok(
                "Missing product fields generated",
                productEnrichmentService.generateMissingFields(productId),
                http.getRequestURI()
        );
    }

    @PostMapping("/{productId}/generate-description")
    public ApiResponse<ProductEnrichmentResponse> generateDescription(
            @PathVariable UUID productId,
            HttpServletRequest http
    ) {
        return ApiResponse.ok(
                "Product description generated",
                productEnrichmentService.generateDescription(productId),
                http.getRequestURI()
        );
    }

    @PostMapping("/{productId}/generate-barcode")
    public ApiResponse<ProductEnrichmentResponse> generateBarcode(
            @PathVariable UUID productId,
            HttpServletRequest http
    ) {
        return ApiResponse.ok(
                "Product barcode generated",
                productEnrichmentService.generateBarcode(productId),
                http.getRequestURI()
        );
    }

    @PostMapping("/{productId}/generate-qr")
    public ApiResponse<ProductEnrichmentResponse> generateQr(
            @PathVariable UUID productId,
            HttpServletRequest http
    ) {
        return ApiResponse.ok(
                "Product QR generated",
                productEnrichmentService.generateQr(productId),
                http.getRequestURI()
        );
    }

    @GetMapping("/{productId}/barcode-image")
    public ResponseEntity<byte[]> barcodeImage(@PathVariable UUID productId) {
        return png(productEnrichmentService.barcodeImage(productId), "product-" + productId + "-barcode.png");
    }

    @GetMapping("/{productId}/qr-image")
    public ResponseEntity<byte[]> qrImage(@PathVariable UUID productId) {
        return png(productEnrichmentService.qrImage(productId), "product-" + productId + "-qr.png");
    }
    ...
    public ApiResponse<Void> delete(@PathVariable UUID productId, HttpServletRequest http) {
        ...
        return ApiResponse.ok("Product moved to trash", null, http.getRequestURI());
    }
    ...
    public ApiResponse<ProductMediaDto> updateMedia(
            @PathVariable UUID productId,
        ...
        return ApiResponse.ok(
                "Media updated",
                unifiedCatalogService.updateMedia(productId, mediaId, request),
                http.getRequestURI()
        );
    }
    ...
    public ApiResponse<Void> deleteMedia(
        ...
        unifiedCatalogService.deleteMedia(productId, mediaId);
        return ApiResponse.ok("Media deleted", null, http.getRequestURI());
    }
