...
public class OrderServiceImpl implements OrderService {
    ...
    public Page<OrderDto> adminOrders(Pageable pageable) {
        return adminOrders(pageable, null, null, null);
    }

    @Override
    @PreAuthorize("hasRole('ADMIN')")
    public Page<OrderDto> adminOrders(Pageable pageable, String query, OrderStatus status, RefundStatus refundStatus) {
        ...
        return orderRepository.findAll(specification, pageable).map(this::mapWithItems);
    }
    ...
    private void notifyOrderStatusChange(Order order, OrderStatus previousStatus) {
        ...
        userAccountRepository.findByRole(RoleType.ADMIN).stream()
                ...
                .forEach(adminId -> notificationCommandPort.pushToUser(
                        adminId,
    ...
    /**
     * Builds query specification.
     *
     * @param query The query value.
     * @return The result of build query specification.
     */
    private Specification<Order> buildQuerySpecification(String query) {
        String wildcardQuery = "%" + query.toLowerCase(Locale.ROOT) + "%";
        Specification<Order> textSpecification = (root, ignoredQuery, cb) -> cb.or(
                cb.like(cb.lower(root.join("user").get("email")), wildcardQuery),
                cb.like(cb.lower(root.join("user").get("fullName")), wildcardQuery),
                cb.and(
                        cb.isNotNull(root.get("couponCode")),
                        cb.like(cb.lower(root.get("couponCode")), wildcardQuery)
                )
        );
        ...
        return textSpecification.or((root, ignoredQuery, cb) -> cb.or(
                ...
                cb.equal(root.join("user").get("id"), parsedUuid)
        ));
    }
     ...
     * @return The result of parses uuid or null.
     */
